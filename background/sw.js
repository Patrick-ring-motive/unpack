...
// Import Stash class
import Stash from '../lib/Stash.js';
...
// Initialize Stash instance in service worker
const stash = new Stash();
globalThis.stash = stash;

// Load and execute service worker code from storage
chrome.storage.sync.get(['swCode'], (result) => {
  if (result.swCode) {
    console.log('Executing service worker code:', result.swCode);
    try {
      interpreter.run(result.swCode);
    } catch (error) {
      console.error('Error executing service worker code:', error);
    }
  }
});

// Listen for service worker code updates and re-execute
chrome.storage.onChanged.addListener((changes, namespace) => {
  if (namespace === 'sync' && changes.swCode) {
    console.log('Service worker code updated, executing new code...');
    try {
      interpreter.run(changes.swCode.newValue);
    } catch (error) {
      console.error('Error executing updated service worker code:', error);
    }
  }
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  console.log('Message received in background:', request);
    // Open a new tab and click a selector using scripting.executeScript
    if (request.action === 'openAndClick') {
        const url = request.url || 'about:blank';
        const selector = request.selector || request.selector || 'button';
        chrome.tabs.create({ url, active: true }, (tab) => {
            const tabId = tab.id;
            // Wait until the tab is fully loaded, then inject script to click
                    ...
                    chrome.scripting.executeScript(
                        ...
                        (results) => {
                            if (chrome.runtime.lastError) {
                                ...
                                sendResponse({ success: true, result: results && results[0] && results[0].result });
                            }
                        }
                    );
                }
    ...
    // Optionally: open tab and click using chrome.debugger (dispatch mouse event)
    // Requires 'debugger' permission. This method simulates a real mouse event at element center.
    if (request.action === 'openAndClickWithDebugger') {
        const url = request.url || 'about:blank';
        const selector = request.selector || 'button';
        chrome.tabs.create({ url, active: true }, (tab) => {
            const tabId = tab.id;
            const attachAndClick = async () => {
                try {
                    ...
                    chrome.debugger.attach(debuggee, '1.3', () => {
                        ...
                        // Evaluate code to get bounding box of the element
                        chrome.debugger.sendCommand(debuggee, 'DOM.enable', {}, () => {
                            chrome.debugger.sendCommand(debuggee, 'Runtime.evaluate', { expression: `(function(){ const el = document.querySelector(${JSON.stringify(selector)}); if(!el) return {__err: 'selector-not-found'}; const r = el.getBoundingClientRect(); return {x: r.left + r.width/2, y: r.top + r.height/2}; })()` , returnByValue: true }, (res) => {
                                if (res && res.result && res.result.value && res.result.value.__err) {
                                    chrome.debugger.detach(debuggee);
                                    sendResponse({ success: false, error: res.result.value.__err });
                                    return;
                                }
                                const coords = res && res.result && res.result.value;
                                if (!coords) {
                                    chrome.debugger.detach(debuggee);
                                    sendResponse({ success: false, error: 'could-not-get-coords' });
                                    return;
                                }
                                // Dispatch mouse events to simulate click
                                chrome.debugger.sendCommand(debuggee, 'Input.dispatchMouseEvent', { type: 'mouseMoved', x: coords.x, y: coords.y, button: 'left', clickCount: 0 }, () => {
                                    chrome.debugger.sendCommand(debuggee, 'Input.dispatchMouseEvent', { type: 'mousePressed', x: coords.x, y: coords.y, button: 'left', clickCount: 1 }, () => {
                                        chrome.debugger.sendCommand(debuggee, 'Input.dispatchMouseEvent', { type: 'mouseReleased', x: coords.x, y: coords.y, button: 'left', clickCount: 1 }, () => {
                                            chrome.debugger.detach(debuggee, () => {
  ...
  if (request.action === 'backgroundTask') {
    ...
    sendResponse({ status: 'success', data: 'Task completed' });
  }

  // Content script asks the debugger to click a selector on its own tab.
  // The tab id is taken from sender so the page never needs to know its own id.
  if (request.action === 'debuggerClickOnSelf') {
    ...
    const detachAndRespond = (payload) => {
      chrome.debugger.detach(debuggee, () => sendResponse(payload));
    };
    ...
    chrome.debugger.attach(debuggee, '1.3', () => {
      ...
      const expr = `(function(){
        ...
        if (!el) return { __err: 'selector-not-found' };
        const r = el.getBoundingClientRect();
        return { x: r.left + r.width / 2, y: r.top + r.height / 2 };
      })()`;
      chrome.debugger.sendCommand(debuggee, 'Runtime.evaluate', { expression: expr, returnByValue: true }, (res) => {
        if (chrome.runtime.lastError) {
          detachAndRespond({ success: false, error: chrome.runtime.lastError.message });
          return;
        }
        const val = res && res.result && res.result.value;
        if (!val || val.__err) {
          detachAndRespond({ success: false, error: (val && val.__err) || 'could-not-get-coords' });
          return;
        }
        const { x, y } = val;
        const mouseEvent = (type, extra = {}) =>
          new Promise(resolve =>
            chrome.debugger.sendCommand(debuggee, 'Input.dispatchMouseEvent',
              { type, x, y, button: 'left', clickCount: 1, ...extra }, resolve));
        mouseEvent('mouseMoved', { clickCount: 0 })
          .then(() => mouseEvent('mousePressed'))
          .then(() => mouseEvent('mouseReleased'))
          .then(() => detachAndRespond({ success: true }))
          .catch(err => detachAndRespond({ success: false, error: err.message }));
      });
    });
    return true; // async response
  }

  // Handle Stash operations from content scripts
  if (request.action === 'stash.get') {
    stash.get(request.key).then(value => {
  ...
  if (request.action === 'stash.set') {
    stash.set(request.key, request.value).then(() => {
      sendResponse({ success: true });
    }).catch(error => {
      sendResponse({ success: false, error: error.message });
    });
  ...
  if (request.action === 'stash.delete') {
    stash.delete(request.key).then(() => {
      sendResponse({ success: true });
    }).catch(error => {
      sendResponse({ success: false, error: error.message });
    });
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === 'extensionContextMenu') {
...
async function debuggerFocusElement(tabId, selector) {
...
async function debuggerTypeKeys(tabId, text) {
  await chrome.debugger.attach({ tabId }, '1.3');
  try {
    for (const char of text) {
      const keyParams = {
        windowVirtualKeyCode: char.charCodeAt(0),
        text: char,
      };
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'debuggerFocusOnSelf') {
    debuggerFocusElement(sender.tab.id, request.selector)
      .then(r  => sendResponse({ success: true,  result: r }))
      .catch(e => sendResponse({ success: false, error: e.message }));
    return true;
  }

  if (request.action === 'debuggerTypeOnSelf') {
    debuggerTypeKeys(sender.tab.id, request.text)
    ...
    scriptRunner.src = URL.createObjectURL(new Blob([`(${pageRunner})();`]))
    document.firstElementChild.appendChild(scriptRunner);*/