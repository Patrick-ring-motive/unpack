          ...
          chrome.debugger.attach(debuggee, '1.3', () => {
            ...
            chrome.debugger.sendCommand(debuggee, 'DOM.enable', {}, () => {
              chrome.debugger.sendCommand(debuggee, 'Runtime.evaluate', {
                expression: `(function(){ const el = document.querySelector(${JSON.stringify(selector)}); if(!el) return {__err: 'selector-not-found'}; const r = el.getBoundingClientRect(); return {x: r.left + r.width/2, y: r.top + r.height/2}; })()`,
                returnByValue: true
              }, (res) => {
    ...
    chrome.debugger.attach(debuggee, '1.3', () => {
      ...
      chrome.debugger.sendCommand(debuggee, 'Runtime.evaluate', {
        ...
        if (!val || val.__err) {
          detachAndRespond({
            success: false,
            error: (val && val.__err) || 'could-not-get-coords'
          });
          return;
        }
        const {
        ...
        mouseEvent('mouseMoved', {
          ...
          .then(() => detachAndRespond({
            success: true
          }))
          .catch(err => detachAndRespond({
            success: false,
            error: err.message
          }));
      });
